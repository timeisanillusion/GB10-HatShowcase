...
from .detection import create_detection_backend, DetectionBackend
...
default_vlm_config = {  # Set at startup; used to create new sessions
    ...
    "detection_prompt": "person,hat",
}
sessions = {}  # session_id -> {"vlm_service": VLMService, "detection_backend": DetectionBackend, "processor_tracks": set}
...
def get_or_create_session(session_id: str):
    ...
    if session_id not in sessions:
        ...
        # Create detection backend if configured
        detection_type = cfg.get("detection_type", "yolo")
        detection_backend = None
        try:
            detection_backend = create_detection_backend(
                backend_type=detection_type,
                model_name=cfg.get("model_name", "yolov8n"),
                prompt=cfg.get("detection_prompt", "person,hat"),
            )
            logger.info(f"Created {detection_type} detection backend")
        except Exception as e:
            logger.warning(f"Could not create detection backend: {e}")
...
async def offer(request):
    ...
    async def on_connectionstatechange():
        ...
        if pc.connectionState in ["failed", "closed"]:
            ...
            # Remove processor tracks from session tracking
            for proc_track in session.get("processor_tracks", set()):
                try:
                    proc_track.stop()
                except Exception:
                    pass
            session.get("processor_tracks", set()).clear()
            await pc.close()
            pcs.discard(pc)
    ...
    if rtsp_url:
        ...
        try:
            ...
            session["processor_tracks"].add(processor_track)
...
async def rtsp_start(request):
    ...
    try:
        ...
        session_callback = get_session_callback(session_id)
        processor_track = VideoProcessorTrack(
            rtsp_track, session_vlm, text_callback=session_callback, detection_backend=session.get("detection_backend")
        )
...
async def _stop_rtsp_session(session_id: str):
    ...
    if session_id in rtsp_tracks:
        ...
        # Remove processor track from session tracking
        session = sessions.get(session_id)
        if session:
            session["processor_tracks"].discard(processor_track)
...
def main():
    ...
    default_vlm_config = {
        ...
        "api_key": api_key,
        "prompt": args.prompt,
        "detection_type": "yolo",  # Use standard YOLO for person detection
        "model_name": "yolov8s.pt",  # Standard YOLO v8 small model
        "detection_prompt": "person,hat",  # Prompt for detection classes
    }
    sessions["default"] = {
